...
- 今何を勉強しているのか
	- 現在は主にAI技術の実践的な活用方法を学んでいます。「現場で活用するためのAIエージェント入門」を読みながら、DifyやLlamaIndexをなど使って実際にRAGシステムやAIエージェントを構築について学んでおり、業務でどのように活用できるかを模索しています。 AI以外では、Next.jsのチュートリアルに取り組んでおり、理解を深めています。これらの技術も、将来的により良い行政サービスを提供するための技術選択肢として習得しています。
- スキルアップのためにしていることはありますか？
	- 毎日30分など、学習時間を確保することを習慣化しており、忙しい日でも継続できるよう習慣化アプリを活用して管理しています。学習内容は業務で使用している技術の深掘りや、新しいフレームワークの習得など、実務に活かせるものを中心に選んでいます。また、X（旧Twitter）で技術系のアカウントをフォローし、最新の技術動向や業界のトレンドを日常的にキャッチアップするようにしています。特に気になった技術については、実際に手を動かして試してみることで理解を深めています。
- 今までReactでどのような開発をしていましたか？
	- 私はRailsアプリケーションにおいて、部分的にReactコンポーネントを導入する開発を経験しています。具体的には、Rails側でページの基本構造やデータ処理を担当し、ユーザーインタラクションが必要な部分にReactコンポーネントを組み込む形で開発していました。 例えば、フォームの動的な入力フィールド追加、医療機関などのデータの絞り込み検索フォーム、といった機能をReactで実装していました。Rails側からpropsでデータを渡し、Reactでインタラクティブな画面を構築し、必要に応じてReact側からFetch APIでRails APIとやり取りする構成でした。 そのためReactの基本的な概念であるコンポーネント設計、state管理、ライフサイクルメソッドの活用などは一通り習得しております。
- 働き始める場合、どのように活躍、貢献できますか？
	- これまでの開発経験を活かし、Reactを用いた開発ではプロジェクトに参加できます。 また行政DXという、市民の方々が直面する課題を技術で解決するという貴財団のミッションに共感しており、行政特有の要件や制約についても積極的に学び、適切なソリューション提案ができるよう努めます。 また早期のキャッチアップに努め、前職でのドキュメント整備などの経験を活かし、開発プロセスの改善やナレッジ共有にも貢献したいと考えています。
## 技術系
